...
@Slf4j
public class MySql8UploadDownloadSiardProjectIT {

    @Rule
    public TemporaryFolder zippedDownloadedProjectFileTempFolder = new TemporaryFolder();

    @Rule
    public MySQLContainer db = new MySQLContainer<>(DockerImageName.parse("mysql:8.1.0"))
            .withUsername("root")
            .withPassword("test")
            .withCommand("--max-allowed-packet=1000000000")
            .withInitScript(ResourcesLoader.MY_SQL_INIT_SCRIPT);

    @Ignore("Incompatible with MySQL 8.")
    @Test
